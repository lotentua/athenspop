...
The scheduler samples uniformly over explicit feasible departure intervals. It keeps probability-table and logit scheduling out of the public API until those methods are implemented as generic behavior.

Travel-time callables must return strictly positive integer seconds. Seeded reproducibility is guaranteed only when user travel-time callables are deterministic with respect to their inputs and stable external data.
