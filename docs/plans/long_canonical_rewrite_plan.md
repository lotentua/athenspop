...
- The internal data model stores time as integer seconds from a declared diary time origin and never stores pandas timestamps or fractional seconds internally.
...
- The canonical input API is dataframe-first: `SurveyDataset.from_dataframes(trips, persons=None, households=None, *, travel_time_function=None)`.
- The dataframe loader expects canonical time columns to already be integer seconds relative to the diary time origin; clock-string parsing is a separate helper with an explicit `time_origin_clock`, overnight wrap policy, and documented output columns.
- For Athens reproduction, the clock parser uses `time_origin_clock = "04:00"` and wraps times before 04:00 to the next diary day, so a clock time such as `03:30` becomes 84600 seconds after diary start rather than 12600 seconds after civil midnight.
...
- The package will not use concrete timestamps internally. Boundary helpers may parse clock strings, timedeltas, or timestamps, but validated internal objects store integer seconds from the diary time origin.
- For the Athens example, documentation should set the diary time origin to 04:00 local diary time and explain that `departure_second = 0` is valid.
- V1 supports concrete timings through `departure_second + arrival_second`, `departure_second + travel_time_seconds`, and `departure_second + travel_time_function` when `travel_time_function` is supplied at dataframe loading or model construction.
- V1 supports uncertain timings only as a departure window with either `travel_time_seconds` or a construction-time deterministic `travel_time_function`: `earliest_departure_second + latest_departure_second + travel_time_seconds` or `earliest_departure_second + latest_departure_second + travel_time_function`.
...
- The migrated paper source and routing resources are checked in under `examples/athens/data/`; `load_athens_wide_diaries()` verifies 513 raw diaries, 1347 canonical trips, 513 person rows, and 513 household rows.
...
- `uv run python -m examples.athens.reproduce` now writes and validates the current computational artifact set under `examples/athens/output/full`: canonical tables, source hashes, input-stage report, validation report, scheduling diagnostics, scheduled trips, diary summary, episodes, state sequences, compound state sequences, transition counts, substitution costs, dissimilarity matrix, average-linkage matrix, cluster labels, cluster summaries, state distributions, temporal cluster distributions, demographic summaries, SVG figures, dendrogram layout, dendrogram SVG, and manifest.
- The maintained smoke path remains available through `examples.athens.reproduce.write_smoke_artifacts(...)` and `examples/athens_reproduction_smoke.py` for fast tests and documentation examples.
- Current broad validation evidence from the migration slice: `uv run pytest -q tests` reports 65 tests passing, `uv run pytest -q examples\athens\tests` reports 36 tests passing, and `uv run ruff check .`, `uv run ruff format --check .`, `uv run ty check`, `uv run sphinx-build -W -b html docs docs\_build\html`, `uv run python -m examples.athens.smoke`, and `uv run python -m examples.athens.reproduce` have passed.
...
- `athenspop.scheduling`: shared interval realization engine, deterministic seeded sampling, feasibility checks, generic return-home imputation support, and explicit diagnostics for infeasible diaries. Athens-specific empirical return-home imputation lives in `examples.athens.imputation` so the generic scheduler API stays lean.
...
- Concrete timing pattern C: `departure_second`, with `travel_time_function` supplied to the dataframe loader or model constructor.
- Uncertain timing pattern D: `earliest_departure_second`, `latest_departure_second`, `travel_time_seconds`.
- Uncertain timing pattern E: `earliest_departure_second`, `latest_departure_second`, with `travel_time_function` supplied to the dataframe loader or model constructor.
...
- All internal time values are `int` seconds from the diary time origin.
- `0` is valid and means exactly the diary time origin, not missing.
...
### Phase 1: Lock Athens Reproduction Requirements

- Convert the equations and methodological choices from `CSuM2026.zip` into `examples/athens/docs/athens_method_contract.md`.
- Record reproduction invariants: 513 raw diaries, 461 complete demographic records, one infeasible diary, 512 scheduled diaries, diary time-origin clock 04:00, observation window 86400 seconds, bin width 900 seconds, 96 bins, 15 original states, 60 compound state-period labels, 36 reduced state-period labels, one constant insertion/deletion cost, average linkage, and 10-cluster presentation.
...
- Create `examples/athens/docs/athens_output_manifest.md` listing each required figure, table, matrix, and summary output from `CSuM2026.pdf`, its generated artifact path, deterministic inputs/seeds, acceptance rule, and any explicitly deferred output.
...
- Current implementation status: `examples/athens/docs/athens_output_manifest.md` now contains the deterministic full-artifact baseline and release tolerances for counts, shapes, matrix structure, key numerical bounds, demographic summaries, temporal cluster distributions, and non-empty SVG outputs.
...
- Current implementation status: strict reported-trip routing exclusion now yields the 512-diary analysis set; generic scheduler return-home imputation and cropping workflows are covered; and the full paper workflow now applies `examples.athens.imputation.impute_athens_return_home_trips(...)`, fitting observed return-home departures by previous activity purpose and adding 124 synthetic return-home trips with explicit provenance while preserving the single strict reported-trip routing exclusion.
...
- Keep `examples/athens_reproduction_smoke.py` as the maintained miniature canonical pipeline until the full paper notebook sequence is rebuilt; it should run validation, model loading, scheduling, sequence construction, dissimilarity calculation, average-linkage clustering, and dendrogram layout on tiny in-memory data.
- Keep `examples/athens/reproduce.py` as the maintained full artifact-writing workflow for the migrated CSuM2026 dataset; it writes named CSV, JSON, and NumPy outputs matching the paper output manifest shape so hidden notebook state does not become part of the reproducibility contract.
- Keep `examples/athens/inputs.py` as the wide-to-canonical converter scaffold: it converts both the checked-in three-diary fixture and migrated 513-diary source into canonical long-form tables. The migrated travel-time resolver now exists in `examples/athens/travel_time.py`; its default finite-mean policy supports robust generic scheduling of all 513 migrated diaries, while `missing_sample_policy="strict"` preserves the legacy non-finite routing sample for `549_trip_2` and yields the 512-diary paper analysis set.
...
- Current implementation status: `examples/athens/docs/athens_walkthrough.md` is the notebook-like step-by-step guide around the maintained full computational artifact command. It calls the generic library and example functions directly, and generated SVG figures remain reproducible output paths under `examples/athens/output/full/figures` rather than copied into built docs so documentation builds stay fast and independent of the full OM run.
...
- Verify that the Athens example reproduces the expected diary count, sequence dimensions, cost matrix properties, clustering method, and documented figures/tables.
...
7. [x] Convert the paper workflow into a documented step-by-step notebook or notebook-like guide that calls the generic library and example functions rather than reimplementing hidden notebook logic. This is implemented as `examples/athens/docs/athens_walkthrough.md`, checked with a focused runtime probe for the source, conversion, validation, scheduling, imputation, episode, sequence, toy cost, and demographic snippets.
...
- Model tests for seconds from the diary time origin, `departure_second = 0`, optional persons, optional households, metadata preservation, and no accidental pandas timestamp leakage.
