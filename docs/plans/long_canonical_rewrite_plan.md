...
- Current broad validation evidence from the latest implementation slice: `uv run rtk pytest -q tests examples\athens` reports 95 tests passing; `uv run ruff check .`, `uv run ruff format --check .`, `uv run ty check`, `uv run sphinx-build -W -b html docs docs\_build\html`, `uv build`, and `uv run python -m examples.athens.reproduce` with automatic artifact validation have passed.
...
- Convert the equations and methodological choices from `CSuM2026.zip` into `docs/design/athens_method_contract.md`.
...
- Create `docs/design/athens_output_manifest.md` listing each required figure, table, matrix, and summary output from `CSuM2026.pdf`, its generated artifact path, deterministic inputs/seeds, acceptance rule, and any explicitly deferred output.
...
- Current implementation status: `docs/design/athens_output_manifest.md` now contains the deterministic full-artifact baseline and release tolerances for counts, shapes, matrix structure, key numerical bounds, demographic summaries, temporal cluster distributions, and non-empty SVG outputs.
...
- Current implementation status: `docs/user_guide/athens_walkthrough.md` is the notebook-like step-by-step guide around the maintained full computational artifact command. It calls the canonical library and example functions directly, and generated SVG figures remain reproducible output paths under `examples/athens/output/full/figures` rather than copied into built docs so documentation builds stay fast and independent of the full OM run.
