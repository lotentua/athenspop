...
def test_optimal_matching_uses_substitution_when_cheaper_than_indels() -> None:
    ...
    assert optimal_matching_dissimilarity(("A",), ("B",), substitution_cost=costs, indel_cost=1.0) == 0.25
...
def test_optimal_matching_uses_indels_for_shifted_equal_length_sequences() -> None:
    ...
    assert optimal_matching_dissimilarity(("A", "B"), ("B", "C"), substitution_cost=costs, indel_cost=1.0) == 2.0
...
def test_dissimilarity_matrix_matches_scalar_optimal_matching_reference() -> None:
    ...
    states = ("A", "B", "C")
    costs = {(source, target): 0.0 if source == target else 1.25 for source in states for target in states}
    ...
    for row_index, first in enumerate(sequences):
        for column_index, second in enumerate(sequences):
            expected[row_index, column_index] = optimal_matching_dissimilarity(first, second, substitution_cost=costs, indel_cost=1.0)
    np.testing.assert_allclose(matrix, expected)


def test_average_linkage_accepts_precomputed_dissimilarity_matrix() -> None:
    matrix = np.array([[0.0, 1.0, 4.0], [1.0, 0.0, 5.0], [4.0, 5.0, 0.0]], dtype=np.float64)
...
def test_dendrogram_layout_returns_plotting_ready_geometry_without_rendering() -> None:
    matrix = np.array([[0.0, 1.0, 4.0], [1.0, 0.0, 5.0], [4.0, 5.0, 0.0]], dtype=np.float64)
    linkage_matrix = average_linkage(matrix)
