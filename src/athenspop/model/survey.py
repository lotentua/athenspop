...
class Trip:
    ...
    Attributes:
        household_id: Household identifier copied from the input trip row.
        person_id: Person identifier copied from the input trip row.
        trip_id: Trip identifier unique within the trip table identity.
        origin: Origin location label for the movement.
        destination: Destination location label for the movement.
        purpose: Activity state reached after the trip.
        mode: Travel mode label used for sequence states and travel-time lookup.
        departure_second: Concrete departure second from the survey time origin, or `None` when the scheduler still has to realize a departure window.
        arrival_second: Concrete arrival second from the survey time origin, or `None` when it must be derived from a duration or travel-time callable.
        travel_time_seconds: Positive integer travel duration, or `None` when it must be supplied by a travel-time callable.
        departure_window: Optional feasible departure range for trips that are not concrete at load time.
        timing_pattern: Validation-classified timing pattern that explains which timing columns supplied the trip.
        metadata: Additional non-key input columns preserved as immutable typed metadata.
    """
...
class PersonMetadata:
    ...
    Attributes:
        ...
        values: Immutable mapping of additional person table columns after boundary normalization.
    """
...
class HouseholdMetadata:
    ...
    Attributes:
        ...
        values: Immutable mapping of additional household table columns after boundary normalization.
    """
...
class Diary:
    ...
    Attributes:
        ...
        person: Optional respondent metadata matched by `(household_id, person_id)`.
...
class SurveyDataset:
    ...
    Attributes:
        ...
        households: Optional household metadata records that were supplied at the dataframe boundary.
        persons: Optional person metadata records that were supplied at the dataframe boundary.
    """
    ...
    def from_dataframes(
        ...
        *,
        ...
        Args:
            ...
            travel_time_fn: Optional callable returning positive integer travel seconds for timing patterns that provide a departure time but no duration or arrival time.
        ...
        Raises:
            ...
            RuntimeError: If validation reports success but does not provide normalized tables.
            ValueError: If `travel_time_fn` is needed during model construction and returns an invalid value.

        Notes:
            Internal code treats the returned objects as complete and trusted; validation belongs at this dataframe/file boundary.
        """
        result = validate_dataframes(trips, persons=persons, households=households, travel_time_fn=travel_time_fn)
        ...
        for _, row in result.normalized_tables.trips.iterrows():
            trip = _build_trip(row, travel_time_fn=travel_time_fn)
...
def _build_trip(row: pd.Series, *, travel_time_fn: TravelTimeFn | None) -> Trip:
    ...
    if timing_pattern == TimingPattern.DEPARTURE_TRAVEL_TIME_FUNCTION and travel_time_fn is not None and departure_second is not None:
        travel_time_seconds = _resolve_travel_time(travel_time_fn, origin, destination, mode, departure_second)
        arrival_second = departure_second + travel_time_seconds
    metadata = _metadata(
        row,
        exclude={
...
def _resolve_travel_time(
    ...
    result = travel_time_fn(origin, destination, mode, departure_second)
    if isinstance(result, bool) or not isinstance(result, int) or result <= 0:
        raise ValueError(
            f"`travel_time_fn` returned {result!r} for origin={origin!r}, destination={destination!r}, mode={mode!r}, departure_second={departure_second}. It must return a strictly positive integer number of seconds."
        )
    return result
