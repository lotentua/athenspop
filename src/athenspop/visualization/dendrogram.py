...
_DEFAULT_COLORS: Final[tuple[str, ...]] = (
    "#4E79A7",
    "#F28E2B",
    "#E15759",
    "#76B7B2",
...
def plot_cut_dendrogram_state_distribution(
    ...
    *,
    ...
    node_height = (
        resolved_style.label_height
        + len(groups) * resolved_style.panel_height
        + max(0, len(groups) - 1) * resolved_style.panel_gap
    )
    width_units = (
        resolved_style.margin_x * 2
        + resolved_style.node_width
        + max(0, len(leaves) - 1) * resolved_style.leaf_spacing
    )
    height_units = (
        resolved_style.margin_top * 2
        + node_height
        + max_depth * resolved_style.depth_spacing
    )
...
def _draw_edges(
    axes: Axes, node: CutDendrogramNode, style: TemporalDendrogramPlotStyle
) -> None:
...
def _draw_edge(
    axes: Axes,
    parent: CutDendrogramNode,
    child: CutDendrogramNode,
    style: TemporalDendrogramPlotStyle,
) -> None:
...
def _draw_node(
    ...
    text_lines = [
        f"height {node.normalized_height:.2f}",
        f"n {len(node.members)}",
    ]
    ...
    for title, states in context.groups:
        ...
        context.axes.text(
            x + context.style.node_width + 0.04,
            panel_y + context.style.panel_height / 2.0,
            title,
            fontsize=6,
            ha="left",
            va="center",
...
def _draw_panel(
    ...
    *,
    ...
    context.axes.add_patch(
        Rectangle(
            (x, y),
            context.style.node_width,
            context.style.panel_height,
            facecolor="#FFFFFF",
            edgecolor="#202020",
            linewidth=0.6,
        )
...
def _node_anchor_bottom(
    ...
    return (
        x + style.node_width / 2.0,
        y
        + style.label_height
        + panel_count * style.panel_height
        + max(0, panel_count - 1) * style.panel_gap,
    )
