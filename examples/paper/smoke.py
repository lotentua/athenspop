...
from athenspop.model import TravelTimeFn
...
def run_example() -> PaperSmokeOutputs:
...
def run_pipeline(
    ...
    *,
    ...
    validation = validate_dataframes(trips, persons=persons, households=households, travel_time_fn=travel_time_fn)
    validation.report.raise_if_invalid()
    dataset = SurveyDataset.from_dataframes(trips, persons=persons, households=households, travel_time_fn=travel_time_fn)
    ...
    dissimilarity_matrix = paper_dissimilarity_matrix(sequences)
