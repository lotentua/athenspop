# Paper Workflow Walkthrough

This page is the notebook-like walkthrough for the maintained CSuM2026 example. It explains the workflow step by step while keeping the executable contract in `examples.paper.reproduce`, so the example does not depend on hidden notebook state.

## 1. Check The Source Files

The full workflow starts by verifying `CSuM2026.pdf`, `CSuM2026.zip`, and the required LaTeX and figure members inside the ZIP against the hashes recorded in `docs/design/paper_method_contract.md`.

```python
from pathlib import Path

from examples.paper.reproduce import verify_paper_sources
...
The generated artifact is `examples/paper/output/full/source_hashes.json`.
...
```python
from examples.paper.inputs import load_paper_wide_diaries

tables = load_paper_wide_diaries(fixture_travel_time_seconds=None)
...
The full writer saves these tables as `data/trips.csv`, `data/persons.csv`, and `data/households.csv` under `examples/paper/output/full`.
...
travel_time = PaperTravelTimeResolver.from_files(missing_sample_policy="strict")
validation = validate_dataframes(tables.trips, persons=tables.persons, households=tables.households, travel_time_fn=travel_time)

validation.report.raise_if_invalid()
dataset = SurveyDataset.from_dataframes(tables.trips, persons=tables.persons, households=tables.households, travel_time_fn=travel_time)
```
...
scheduled = schedule_once(dataset, seed=2026, config=SchedulingConfig(min_activity_duration_seconds=1800, allow_trips_after_observation_window=True), travel_time_fn=travel_time)
...
The paper-specific return-home policy belongs in `examples.paper.imputation`, not in the generic package API. It fits empirical return-home departure samples from observed return-home trips, samples a feasible synthetic return after the final reported activity when required, and keeps recreation-ending diaries open as described in the paper contract.

```python
from examples.paper.imputation import impute_paper_return_home_trips

return_travel_time = PaperTravelTimeResolver.from_files(missing_sample_policy="finite_mean")
scheduled_with_returns = impute_paper_return_home_trips(scheduled, travel_time_fn=return_travel_time, seed=2026, min_activity_duration_seconds=1800)
```
...
episodes = episodes_from_diary(scheduled_with_returns.diaries[0])
states = state_sequence_from_diary(scheduled_with_returns.diaries[0])
...
## 7. Compute Paper Costs And Dissimilarities

The paper-specific cost construction reduces activities and modes, combines states with four demand periods, excludes self-transitions from transition denominators, builds `c(i, j) = 2 - P(j|i) - P(i|j)`, and uses indel cost `1` for optimal matching.
...
```python
from examples.paper.demographics import demographic_summary
...
The command writes `examples/paper/output/full`, validates the source hashes, validates the generated artifact set against `docs/design/paper_output_manifest.md`, and fails if a release-relevant count, shape, manifest path, matrix property, or imputed-row provenance check no longer matches the documented baseline.

Generated SVG figures are not copied into the built documentation for v1. They remain reproducible output paths under `examples/paper/output/full/figures` so documentation builds stay fast and do not depend on the 512-diary optimal-matching run.
