...
from examples.paper.inputs import load_paper_wide_diaries, load_wide_diary_fixture
...
def test_load_paper_wide_diaries_matches_migrated_source_stage_counts() -> None:
    tables = load_paper_wide_diaries()
...
def test_load_paper_wide_diaries_can_emit_travel_time_function_rows() -> None:
    ...
    result = validate_dataframes(
        ...
        travel_time_fn=resolver,
    )
    assert not result.report.has_errors


def test_migrated_paper_source_schedules_with_cropping_policy() -> None:
    tables = load_paper_wide_diaries()
    dataset = SurveyDataset.from_dataframes(tables.trips, persons=tables.persons, households=tables.households)
...
def test_migrated_paper_source_schedules_with_real_travel_time_resolver() -> None:
    tables = load_paper_wide_diaries(fixture_travel_time_seconds=None)
    resolver = PaperTravelTimeResolver.from_files()
...
def test_migrated_paper_source_strict_routing_matches_legacy_single_exclusion() -> None:
    tables = load_paper_wide_diaries(fixture_travel_time_seconds=None)
    resolver = PaperTravelTimeResolver.from_files(missing_sample_policy="strict")
    dataset = SurveyDataset.from_dataframes(
        tables.trips,
        persons=tables.persons,
        households=tables.households,
