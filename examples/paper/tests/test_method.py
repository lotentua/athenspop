...
from examples.paper.method import (
    compound_period_sequence,
    compound_sequence_from_diary,
    paper_dissimilarity_matrix,
    period_label,
    reduce_paper_state,
    substitution_costs,
    transition_counts,
...
def test_period_label_matches_paper_boundaries(second: int, expected: int) -> None:
    assert period_label(second) == expected


def test_reduced_compound_period_sequence_matches_paper_alphabet() -> None:
    ...
    assert reduce_paper_state("market") == "flexible"
    assert reduce_paper_state("trip_bicycle") == "trip_micromobility"


def test_compound_sequence_from_scheduled_diary_uses_paper_labels() -> None:
    diary = Diary(
        household_id="h1",
        person_id="p1",
        trips=(
...
def test_paper_transition_denominator_intentionally_differs_from_traminer_style() -> None:
    sequence = ("A", "A", "B", "A", "A", "C")
    sequences = (sequence,)
    paper_probabilities = transition_probabilities(sequences)
    traminer_style_probabilities = _self_inclusive_transition_probabilities(sequences)
    assert paper_probabilities[("A", "B")] == 0.5
    assert paper_probabilities[("A", "C")] == 0.5
    assert paper_probabilities[("A", "A")] == 0.0
...
def test_paper_substitution_costs_intentionally_differ_from_traminer_style_trate() -> None:
    sequences = (("A", "A", "B", "A", "A", "C"),)
    paper_costs = substitution_costs(sequences)
    traminer_style_costs = _self_inclusive_substitution_costs(sequences)
    assert paper_costs[("A", "B")] == pytest.approx(0.5)
    assert traminer_style_costs[("A", "B")] == pytest.approx(0.75)
    assert paper_costs[("A", "B")] != traminer_style_costs[("A", "B")]


def test_paper_dissimilarity_fixture_remains_symmetric_and_finite() -> None:
    matrix = paper_dissimilarity_matrix(
        (
            ("A", "A", "B", "A", "A", "C"),
            ("A", "B", "A", "C", "A", "C"),
            ("C", "A", "B", "A", "C", "A"),
