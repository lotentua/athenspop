...
from examples.paper.travel_time import MissingSamplePolicy, PaperTravelTimeResolver


def test_paper_travel_time_resolver_returns_integer_seconds_for_known_od() -> None:
    resolver = _resolver()
...
def test_paper_travel_time_resolver_uses_diary_t0_to_interpolate_civil_hour() -> None:
    resolver = _resolver()
    assert resolver("1", "2", "car", 0) == 1080
    assert resolver("1", "2", "car", 7_200) == 1440


def test_paper_travel_time_resolver_falls_back_for_nan_and_unknown_zones() -> None:
    resolver = _resolver()
...
def test_paper_travel_time_resolver_can_preserve_strict_missing_samples() -> None:
...
def test_paper_travel_time_resolver_loads_files(tmp_path: Path) -> None:
    ...
    resolver = PaperTravelTimeResolver.from_files(routing_path=routing_path, zone_encoder_path=zone_encoder_path)
    assert resolver("1", "1", "walk", 0) == 720


def test_paper_travel_time_resolver_loads_migrated_default_resources() -> None:
    resolver = PaperTravelTimeResolver.from_files()
    assert resolver("2", "15", "train", 14_400) > 0
    assert resolver("15", "2", "walk", 46_800) > 0


def test_paper_travel_time_resolver_rejects_unsupported_modes() -> None:
    resolver = _resolver()
    with pytest.raises(ValueError, match="Unsupported travel mode"):
        resolver("1", "2", "hoverboard", 0)


def _resolver(*, missing_sample_policy: MissingSamplePolicy = "finite_mean") -> PaperTravelTimeResolver:
    return PaperTravelTimeResolver(
