...
PAPER_WINDOW_SECONDS: Final[int] = DEFAULT_WINDOW_SECONDS
PAPER_DELTA_SECONDS: Final[int] = DEFAULT_INTERVAL_SECONDS
...
def period_label(second: int) -> int:
    ...
    if 0 <= second < 3 * 3600 or 15 * 3600 <= second <= PAPER_WINDOW_SECONDS:
        return 1
    ...
    if 12 * 3600 <= second < 15 * 3600:
        return 4
    raise ValueError(f"`second` must be in [0, {PAPER_WINDOW_SECONDS}], got {second}.")
...
def compound_sequence_from_diary(
    ...
    *,
    ...
    window_end_second: int = PAPER_WINDOW_SECONDS,
    interval_seconds: int = PAPER_DELTA_SECONDS,
) -> tuple[str, ...]:
    """Convert one scheduled diary into the reduced compound state-period sequence used by the paper example."""
...
def compound_period_sequence(states: Sequence[str], *, interval_seconds: int = PAPER_DELTA_SECONDS) -> tuple[str, ...]:
    """Reduce states and append CSuM2026 period labels based on each bin start time."""
    return tuple(f"{reduce_paper_state(state)}@p{period_label(index * interval_seconds)}" for index, state in enumerate(states))


def reduce_paper_state(state: str) -> str:
    """Reduce an activity or `trip_` mode state according to the paper example alphabet."""
    ...
    raise ValueError(f"Unknown paper example state {state!r}.")
...
def paper_dissimilarity_matrix(sequences: Sequence[Sequence[str]], *, indel_cost: float = 1.0) -> DissimilarityMatrix:
    """Compute the paper example pairwise OM matrix using local CSuM2026 substitution costs."""
