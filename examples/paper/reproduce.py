...
from examples.paper.demographics import (
    demographic_summary,
    write_bivariate_demographic_svg,
    write_marginal_demographic_svg,
)
from examples.paper.imputation import impute_paper_return_home_trips
from examples.paper.inputs import (
    DEFAULT_PAPER_WIDE_DIARY_PATH,
    PaperInputTables,
    load_paper_wide_diaries,
    load_wide_diary_fixture,
)
from examples.paper.method import substitution_costs, transition_counts
from examples.paper.smoke import PaperSmokeOutputs, run_pipeline
from examples.paper.travel_time import PaperTravelTimeResolver
...
EXPECTED_PAPER_SOURCE_HASHES: Final[dict[str, str]] = {
...
@dataclass(frozen=True, slots=True)
class PaperArtifactExpectations:
...
class PaperArtifactValidation:
    ...
    def raise_if_invalid(self) -> None:
        ...
        if failures:
            ...
            raise ValueError(f"Paper artifact validation failed: {details}")


SMOKE_ARTIFACT_EXPECTATIONS: Final[PaperArtifactExpectations] = PaperArtifactExpectations(
    workflow="paper_smoke",
    raw_diaries=3,
    canonical_trips=9,
    attempted_diaries=3,
    scheduled_diaries=3,
...
FULL_ARTIFACT_EXPECTATIONS: Final[PaperArtifactExpectations] = PaperArtifactExpectations(
    workflow="paper_full",
    raw_diaries=513,
...
def write_smoke_artifacts(
    ...
    *,
    ...
    paths = _write_artifacts(
        ...
        source_report=verify_paper_sources(resolved_source_root),
        outputs=outputs,
        workflow="paper_smoke",
...
def write_full_artifacts(
    ...
    *,
    source_root: Path | None = None,
    wide_diary_path: Path = DEFAULT_PAPER_WIDE_DIARY_PATH,
) -> PaperArtifactPaths:
    ...
    data_dir = root / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    input_tables = load_paper_wide_diaries(wide_diary_path, fixture_travel_time_seconds=None)
    travel_time_resolver = PaperTravelTimeResolver.from_files(missing_sample_policy="strict")
    imputed_return_travel_time_resolver = PaperTravelTimeResolver.from_files(missing_sample_policy="finite_mean")

    def impute_return_home(
        scheduled: ScheduledSurveyDataset,
    ) -> ScheduledSurveyDataset:
        return impute_paper_return_home_trips(
            scheduled,
            travel_time_fn=imputed_return_travel_time_resolver,
            seed=2026,
    ...
    outputs = run_pipeline(
        ...
        travel_time_fn=travel_time_resolver,
    ...
    validate_paper_artifacts(paths, expectations=FULL_ARTIFACT_EXPECTATIONS)
    return paths


def validate_paper_artifacts(paths: PaperArtifactPaths, *, expectations: PaperArtifactExpectations) -> PaperArtifactValidation:
    ...
    if not all(check.passed for check in checks):
        validation = PaperArtifactValidation(paths.root, tuple(checks))
    ...
    checks.extend(
        [
            ...
            _check_equal(
                ...
                tuple(sorted(EXPECTED_PAPER_SOURCE_HASHES)),
            ),
            _check_equal(
...
def _write_artifacts(
    *,
    root: Path,
    input_tables: PaperInputTables,
    wide_diary_path: Path,
    source_report: dict[str, JsonValue],
    outputs: PaperSmokeOutputs,
    workflow: str,
    description: str,
) -> PaperArtifactPaths:
    ...
    state_sequences = tuple(state_sequence_from_diary(diary) for diary in outputs.scheduled.diaries)
...
def main() -> None:
    ...
    paths = write_full_artifacts(Path("examples/paper/output/full"))
    print(f"Wrote full paper artifacts to {paths.root}")


def verify_paper_sources(source_root: Path) -> dict[str, JsonValue]:
    """Verify CSuM2026 PDF, ZIP, and key ZIP member hashes against the method contract."""
    source_root = source_root.resolve()
    source_files: dict[str, JsonValue] = {}
    for filename, expected_hash in EXPECTED_PAPER_SOURCE_HASHES.items():
...
def _artifact_paths(root: Path) -> PaperArtifactPaths:
...
def _named_artifact_paths(paths: PaperArtifactPaths) -> tuple[tuple[str, Path], ...]:
    return (
        ("trips", paths.trips),
...
def _check_path_exists(name: str, path: Path) -> PaperArtifactCheck:
    if path.exists():
        return PaperArtifactCheck(name, True, f"`{path}` exists.")
    return PaperArtifactCheck(name, False, f"`{path}` is missing.")


def _check_equal[ValueT](name: str, actual: ValueT, expected: ValueT) -> PaperArtifactCheck:
    if actual == expected:
        return PaperArtifactCheck(name, True, f"got expected value {expected!r}.")
    return PaperArtifactCheck(name, False, f"expected {expected!r}, got {actual!r}.")


def _check_true(name: str, condition: bool) -> PaperArtifactCheck:
    if condition:
        return PaperArtifactCheck(name, True, "condition is true.")
    return PaperArtifactCheck(name, False, "condition is false.")


def _check_manifest_paths(paths: PaperArtifactPaths, manifest_artifacts: dict[str, JsonValue]) -> tuple[PaperArtifactCheck, ...]:
...
def _expected_manifest_paths(paths: PaperArtifactPaths) -> tuple[tuple[str, Path], ...]:
...
def _validation_report_json(outputs: PaperSmokeOutputs) -> dict[str, JsonValue]:
...
def _scheduling_diagnostics_json(outputs: PaperSmokeOutputs) -> dict[str, JsonValue]:
...
def _episodes_by_diary(
    ...
    return tuple((diary.household_id, diary.person_id, episodes_from_diary(diary)) for diary in diaries)
...
def _cluster_labels_frame(outputs: PaperSmokeOutputs) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "household_id": diary.household_id,
...
def _dendrogram_json(outputs: PaperSmokeOutputs) -> dict[str, JsonValue]:
...
def _input_stage_report_json(input_tables: PaperInputTables, wide_diary_path: Path) -> dict[str, JsonValue]:
    note = (
