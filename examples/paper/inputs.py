...
PAPER_T0_HOUR: Final[int] = 4
DEFAULT_FIXTURE_TRAVEL_TIME_SECONDS: Final[int] = 900
DEFAULT_PAPER_WIDE_DIARY_PATH: Final[Path] = Path(__file__).resolve().parent / "data" / "raw_diaries_athens_wide.csv"
...
def load_paper_wide_diaries(
    path: Path = DEFAULT_PAPER_WIDE_DIARY_PATH,
    *,
    fixture_travel_time_seconds: int | None = DEFAULT_FIXTURE_TRAVEL_TIME_SECONDS,
) -> PaperInputTables:
...
def _departure_window_seconds(absolute_hour: int) -> tuple[int, int]:
    ...
    return (
        max(0, (start_hour - PAPER_T0_HOUR) * SECONDS_PER_HOUR),
        max(0, (end_hour - PAPER_T0_HOUR) * SECONDS_PER_HOUR),
    )
