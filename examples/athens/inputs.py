...
DEFAULT_ATHENS_WIDE_DIARY_PATH: Final[Path] = (
    Path(__file__).resolve().parent / "data" / "raw_diaries_athens_wide.csv"
)
...
def load_wide_diary_fixture(
    ...
    *,
    ...
    return wide_diaries_to_canonical(
        wide, fixture_travel_time_seconds=fixture_travel_time_seconds
    )
...
def wide_diaries_to_canonical(
    ...
    *,
    ...
    trips = _trips_from_wide(
        usable, fixture_travel_time_seconds=fixture_travel_time_seconds
    )
...
def _validate_wide_columns(wide: pd.DataFrame) -> None:
    ...
    if missing:
        raise ValueError(
            f"The wide diary table is missing required column(s): {', '.join(missing)}."
        )
...
def _trips_from_wide(
    wide: pd.DataFrame, *, fixture_travel_time_seconds: int | None
) -> pd.DataFrame:
...
def _departure_window_seconds(absolute_hour: int) -> tuple[int, int]:
    ...
    if hour >= 23:
        ...
        matching_window = next(
            window for window in TIME_WINDOWS if window[0] <= hour < window[1]
        )
...
def _hour(value: RawCell) -> int:
    if pd.isna(value):
        raise ValueError(
            "Reported departure hour cannot be missing for a reported trip."
        )
